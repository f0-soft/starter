...
function init( config, callback ) {
	...
	if ( INITIALIZED ) {
		return next( callback, myErr( 'Повторная инициализация системы запрещена' ) );
	}
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		if ( invalid ) {
			return next( callback, myErr( 'link `' + files[i] + '` - ' + invalid ) );
		}
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		for ( j = 0; j < dict.mutable.length; j += 1 ) {
			...
			if ( elem.from && elem.link ) {
				if ( links_dict[elem.link] === undefined ) {
					return next( callback, myErr( 'flexo `' + file.name + '`, field `' + dict.mutable[j] + '` - no such link' ) );
				}

				if ( links_dict[elem.link][0][0] !== file.name && links_dict[elem.link][1][0] !== file.name ) {
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		while ( queue.length > 0 ) {
			...
			if ( invalid ) { return next( callback, myErr( 'view `' + files[i] + '` - ошибка `' + invalid + '` в элементе конфига `' + JSON.stringify( elem ) + '`' ) ); }
			...
			if ( elem._vid !== undefined ) {
				...
				if ( elem._flexo ) {
					...
					if ( VID_TYPES.indexOf( elem._flexo.type ) === -1 ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - недопустимое значение поля `type`' ) ); }
					...
					if ( elem._flexo.scheme ) {
						...
						if ( flexo_dict[ elem._flexo.scheme[0] ] === undefined ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - не существует указанной схемы flexo' ) ); }
						// указано поле связи для некорневой схемы
						if ( elem._flexo.scheme[0] !== file.root && elem._flexo.scheme[2] === undefined ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - `depend_field` обязателен для некорневых схем' ) )}

						dict[ elem._vid ].flexo = [ elem._flexo.scheme[0] ];

						if ( elem._flexo.type !== VID_TYPES[3] ) { // not 'delete'
							// наличие поля схемы
							if ( elem._flexo.scheme[1] === undefined ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - `field` обязателен (второй элемент массива)' ) ); }
							// наличие поля в схеме
							if ( flexo_dict[ elem._flexo.scheme[0] ].dict.all.indexOf( elem._flexo.scheme[1] ) === -1 ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - в схеме flexo отсутствует указанный `field` (второй элемент массива)' ) ); }
							...
							if ( elem._flexo.scheme[2] ) {
								...
								if ( flexo_dict[file.root].dict.all.indexOf( elem._flexo.scheme[2] ) === -1 ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - указанного `depend_field` не существует (третий элемент массива)' ) );}
								// наличие названия пути в связях через 1+
								if ( flexo_dict[file.root].dict.types[ elem._flexo.scheme[2] ].from !== elem._flexo.scheme[0] && !elem._flexo.scheme[3] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - для связи через промежуточные схемы `link` обязателен (четвертый элемент массива)' ) )}
								...
								if ( elem._flexo.scheme[3] ) {
									...
									if ( !links_dict[ elem._flexo.scheme[3] ] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - указанного `link` не существует (четвертый элемент массива)' ) ); }
					...
					if ( elem._flexo.aggregate ) {
						if ( !file.aggregate ) { return next( callback, myErr( 'view `' + file.name + '` - в схеме отсутствует блок `aggregate` необходимый элементам конфига' ) );}
						if ( !file.aggregate[elem._flexo.aggregate.name] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - не существует указанного идентификатора агрегации `name`' ) );}
						if ( (!elem._flexo.aggregate.group && !elem._flexo.aggregate.selector) || (elem._flexo.aggregate.group !== undefined && elem._flexo.aggregate.selector !== undefined) ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - должно присутствовать одно из полей: `group` или `selector`' ) );}
