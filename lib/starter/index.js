...
function init( config, callback ) {
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		console.log( SETTINGS.rabbit.config.gFieldDepPath );
			...
			if ( elem._vid ) {
				...
				if ( elem._flexo ) {
					...
					if ( elem._flexo.scheme[2] ) {
						if ( !elem._flexo.scheme[3] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - path name required' ) ); }
						if ( !links_dict[ elem._flexo.scheme[3] ] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - no such path' ) ); }
					}
