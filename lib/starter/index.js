...
function init( config, callback ) {
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		while ( queue.length > 0 ) {
			...
			if ( invalid ) { return next( callback, myErr( 'view `' + files[i] + '` - ' + invalid + ' on config element ' + JSON.stringify( elem ) ) ); }

			if ( elem._vid ) {
				dict[ elem._vid ] = {};

				if ( elem._flexo ) {
					if ( flexo_dict[ elem._flexo.scheme[0] ] === undefined ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - no such flexo' ) ); }
					if ( VID_TYPES.indexOf( elem._flexo.type ) === -1 ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - wrong type' ) ); }

					dict[ elem._vid ].flexo = [ elem._flexo.scheme[0] ];
					dict[ elem._vid ].type = elem._flexo.type;

					if ( elem._flexo.type !== VID_TYPES[3] ) { // not 'delete'
						if ( typeof elem._flexo.scheme[1] !== 'string' ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - field required' ) ); }
						if ( flexo_dict[ elem._flexo.scheme[0] ].dict.all.indexOf( elem._flexo.scheme[1] ) === -1 ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - no such field in flexo' ) ); }

						// add field
						dict[ elem._vid ].flexo[1] = elem._flexo.scheme[1];

						if ( elem._flexo.scheme[2] ) {
							if ( !elem._flexo.scheme[3] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - link required' ) ); }
							if ( !links_dict[ elem._flexo.scheme[3] ] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - no such link' ) ); }

							// add depend_field and link
							dict[ elem._vid ].flexo[2] = elem._flexo.scheme[2];
							dict[ elem._vid ].flexo[3] = links_dict[ elem._flexo.scheme[3] ];
						}
					}
				}
			}
