...
function init( config, callback ) {
	...
	var links_dict, flexo_dict, dict, flexoTypes;
	var i, j, k, keys, subkeys, fields, field, queue;
	...
	// Копирование модулей
	// ----------------------------------
	modules = ['rabbit-server', 'rabbit', 'flexo', 'flexo-client', 'view', 'controller'];
	...
	if ( config.redis !== undefined ) {
		SETTINGS['rabbit-server'].config.redis = config.redis;
		SETTINGS.controller.config.redisConfig = config.redis;
	}
	if ( config.mongo !== undefined ) {
		SETTINGS['rabbit-server'].config.mongo = config.mongo;
	}
	if ( config.rabbit_host !== undefined ) {
		SETTINGS['rabbit-server'].config.host = config.rabbit_host;
		SETTINGS.rabbit.config.host = config.rabbit_host;
	}
	if ( config.rabbit_port !== undefined ) {
		SETTINGS['rabbit-server'].config.port = config.rabbit_port;
		SETTINGS.rabbit.config.port = config.rabbit_port;
	}
	SETTINGS.view.config.templatePath = config.template_path;
	SETTINGS.view.config.templateTimeout = config.template_timeout;
	SETTINGS.flexo.config.server.port = config.flexo_port;
	SETTINGS['flexo-client'].config.host = config.flexo_host;
	SETTINGS['flexo-client'].config.port = config.flexo_port;
	...
	// rabbit-server
	SETTINGS['rabbit-server'].config.gPrefixCol = dict;
	...
	// Копирование путей
	// ----------------------------------
	paths = ['flexo_path', 'type_path', 'link_path', 'view_path', 'template_path'];
	...
	for ( i = 0; i < files.length; i += 1 ) {
		if ( files[i][0] === '.' ) { continue; } // игнорирование файлов, начинающихся с точки
		try {
	...
	// rabbit-server
	SETTINGS['rabbit-server'].config.gPath = links_dict;
	...
	// ----------------------------------
	// Подключение типов flexo
	// ----------------------------------
	checks.flexoType = argstype.getChecker( plainErr, [
		['type', true, 'o', [
			['name', true, 's'],
			['array', false, 'b'],
			['subtype', false, 'b'],
	...
	flexoTypes = {};
	files = fs.readdirSync( SETTINGS.type_path );
	for ( i = 0; i < files.length; i += 1 ) {
		if ( files[i][0] === '.' ) { continue; } // игнорирование файлов, начинающихся с точки
		try {
			file = require( SETTINGS.type_path + files[i] );
		} catch ( e ) {
			return next( callback, myErr( 'type, file `' + files[i] + '` - ' + e.message ) );
		}

		invalid = checks.flexoType( [file] );
		if ( invalid ) { return next( callback, myErr( 'type, file `' + files[i] + '` - ' + invalid ) ); }

		if ( flexoTypes[ file.name ] ) { return next( callback, myErr( 'type, file `' + files[i] + '`, name - такое имя уже используется другим типом данных' ) ); }

		flexoTypes[ file.name ] = file;
	}

	// flexo
	SETTINGS.flexo.config.types = flexoTypes;
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		// проверка названия схемы
		var restrictedCollectionChars = /(\$|^system\.)/;
		if ( restrictedCollectionChars.test( file.name ) ) { return next( callback, myErr( 'flexo, схема `' + files[i] + '`, name - использованы недопустимые символы' ) ); }
		if ( !file.name.length ) { return next( callback, myErr( 'flexo, схема `' + files[i] + '`, name - не может быть пустой строкой' ) ); }
		...
		keys = dict.mutable;
		var restrictedFieldChars = /(\.|\$)/;
		for ( j = 0; j < keys.length; j += 1 ) {
			if ( restrictedFieldChars.test( keys[j] ) ) { return next( callback, myErr( 'flexo, схема `' + files[i] + '`, `' + keys[j] + '` - использован недопустимый символ' ) ); }
			if ( !keys[j].length ) { return next( callback, myErr( 'flexo, схема `' + files[i] + '`, `' + keys[j] + '` - не может быть пустой строкой' ) ); }
		}
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			if ( !flexoTypes[ file.root[keys[j]].type ] ) {
				return next( callback, myErr( 'flexo `' + file.name + '`, field `' + keys[j] + '`, type `' + file.root[keys[j]].type + '` - нет такого типа данных' ) );
			}

			if ( file.root[keys[j]].type === 'objarray' && !file.root[keys[j]].subtype ) {
				return next( callback, myErr( 'для типа `objarray` обязательно указание вложенной схемы посредством поля `subtype`' ) );
			}

			if ( file.root[keys[j]].subtype ) {
				subkeys = Object.keys( file.root[keys[j]].subtype );
				for ( k = 0; k < subkeys.length; k += 1 ) {
					if ( !flexoTypes[ file.root[keys[j]].subtype[subkeys[k]].type ] ) {
						return next( callback, myErr( 'flexo `' + file.name + '`, field `' + keys[j] + '.' + subkeys[k] + '`, type `' + file.root[keys[j]].subtype[subkeys[k]].type + '` - нет такого типа данных' ) );
					}
				}
			}
		...
		for ( j = 0; j < dict.mutable.length; j += 1 ) {
			...
			if ( elem.type === 'words' ) {
				SETTINGS['rabbit-server'].config.gWordFields[ file.name ] = SETTINGS['rabbit-server'].config.gWordFields[ file.name ] || {};
				SETTINGS['rabbit-server'].config.gWordFields[ file.name ][ dict.mutable[j] ] = 1;
			}

			if ( flexoTypes[ elem.type ].array ) {
				SETTINGS['rabbit-server'].config.gArrayFields[ file.name ] = SETTINGS['rabbit-server'].config.gArrayFields[ file.name ] || {};
				SETTINGS['rabbit-server'].config.gArrayFields[ file.name ][ dict.mutable[j] ] = 1;

				if ( flexoTypes[ elem.type ].subtype ) {
					SETTINGS['rabbit-server'].config.gArrayFields[ file.name ][ dict.mutable[j] ] = {};
					keys = Object.keys( dict.types[ dict.mutable[j] ].subtype );
					for ( k = 0; k < keys.length; k += 1 ) {
						SETTINGS['rabbit-server'].config.gArrayFields[ file.name ][ dict.mutable[j] ][ keys[k] ] = 1;
					}
				}
			}
			...
			if ( elem.from ) {
				...
				if ( elem.link && links_dict[elem.link][0][0] !== file.name && links_dict[elem.link][1][0] !== file.name ) {
					if ( !SETTINGS['rabbit-server'].config.gFieldDepPath[ file.name ] ) {
						SETTINGS['rabbit-server'].config.gFieldDepPath[ file.name ] = {};
					}

					SETTINGS['rabbit-server'].config.gFieldDepPath[ file.name ][ dict.mutable[j] ] = [ elem.link, elem.from ];
				}

				if ( SETTINGS['rabbit-server'].config.gBackRef[elem.from] === undefined ) {
					SETTINGS['rabbit-server'].config.gBackRef[elem.from] = [];
				}

				SETTINGS['rabbit-server'].config.gBackRef[elem.from].push( [ file.name, dict.mutable[j] ] );
			}

			if ( elem.weight ) {
				SETTINGS['rabbit-server'].config.gScore[ file.name ] = SETTINGS['rabbit-server'].config.gScore[ file.name ] || {};
				SETTINGS['rabbit-server'].config.gScore[ file.name ][ dict.mutable[j] ] = 1;
			}
			...
			if ( elem.includeWeight ) {
				...
				SETTINGS['rabbit-server'].config.gScoreJoin[ file.name ] = SETTINGS['rabbit-server'].config.gScoreJoin[ file.name ] || {};
				SETTINGS['rabbit-server'].config.gScoreJoin[ file.name ][ dict.mutable[j] ] = {};
				for ( k = 0; k < elem.includeWeight.length; k += 1 ) {
					if ( elem.includeWeight[k] !== elem.from && elem.link === undefined ) {
						return next( callback, myErr( 'flexo `' + file.name + '`, field `' + dict.mutable[j] + '` - для включения веса из несмежной коллекции, необходимо указать `link`' ) );
					}
					...
					SETTINGS['rabbit-server'].config.gScoreJoin[ file.name ][ dict.mutable[j] ][ elem.includeWeight[k] ] = 1;
				}
			}
		}
			...
			if ( dict[ keys[j] ].aggregate !== undefined ) {
				...
				if ( file.aggregate[dict[ keys[j] ].aggregate.name].flexo !== undefined ) {
					...
					if ( dict[ keys[j] ].aggregate.selector !== undefined ) {
						...
						field = dict[ keys[j] ].aggregate.group[Object.keys( dict[ keys[j] ].aggregate.group )[0]];
						if ( typeof field === 'string' ) {
							SETTINGS.controller.config.viewConfig[ file.name ][ keys[j] ].flexo[1] = field.substr( 1 );
						}
					}
				}
...
function start( callback ) {
	var allModules = {};
	async.waterfall( [
		function( cb ) { // rabbit-server
			log( 'Starter, Rabbit-server config:', SETTINGS['rabbit-server'].config );
			SETTINGS['rabbit-server'].module.init( SETTINGS['rabbit-server'].config, cb );
		},
		function( res, cb ) { // rabbit
			allModules['rabbit-server'] = SETTINGS['rabbit-server'].module;
		...
		function( module, cb ) { // flexo-client
			allModules.flexo = module;
			log( 'Starter, Flexo-client config:', SETTINGS['flexo-client'].config );
			SETTINGS['flexo-client'].module.init( SETTINGS['flexo-client'].config, cb );
		},
		function( module, cb ) { // view
			allModules['flexo-client'] = module;
			SETTINGS.view.config.provider = allModules['flexo-client'];
