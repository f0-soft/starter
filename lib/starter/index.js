...
var ROOT_SYSTEM_PROPERTIES = { '_id': { type: 'id' }, 'tsCreate': { type: 'int' }, 'tsUpdate': { type: 'int' } };
...
function init( config, callback ) {
	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		// nest types
		dict.types = {_id: {type: 'id'}, tsCreate: {type: 'int'}, tsUpdate: {type: 'int'}};
		...
		for ( j = 0; j < dict.mutable.length; j += 1 ) {
			...
			if ( elem.from ) {
				if ( elem.link && dict.types[ dict.mutable[j] ].type !== 'idpath' ) {
					return next( callback, myErr( 'flexo `' + file.name + '`, field `' + dict.mutable[j] + '` - поле, ссылающееся на другую коллекцию через путь, должно быть типа `idpath`' ) );
				}
