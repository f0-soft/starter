	...
	for ( i = 0; i < files.length; i += 1 ) {
		...
		while ( queue.length > 0 ) {
			...
			if ( elem._vid ) {
				...
				if ( elem._flexo ) {
					...
					if ( elem._flexo.title ) { dict[ elem._vid ].title = elem._flexo.title; }
					if ( elem._flexo.description ) { dict[ elem._vid ].description = elem._flexo.description; }
					...
					if ( elem._flexo.scheme ) {
						...
						if ( elem._flexo.type !== VID_TYPES[3] ) { // not 'delete'
							if ( typeof elem._flexo.scheme[1] !== 'string' ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - `field` обязателен (второй элемент массива)' ) ); }
							...
							if ( elem._flexo.scheme[2] ) {
								if ( !elem._flexo.scheme[3] ) { return next( callback, myErr( 'view `' + file.name + '`, vid `' + elem._vid + '` - `link` обязателен (четвертый элемент массива)' ) ); }
		...
		for ( j = 0; j < keys.length; j += 1 ) {
			SETTINGS.controller.config.viewConfig[ file.name ][ keys[j] ] = {
				title: dict[ keys[j] ].title || keys[j],
				description: dict[ keys[j] ].description || ''
			};
